...
class BlogAPI extends database
{
    ...
    public function getBlogs($id)
    {
        $a=[];
        $q=[];
        if($this->Query("SELECT p.heading,p.type,p.description,p.approval_status,p.datetime,p.likes,p.comments,a.url FROM post p left join post_attachments a on p.id=a.post_id where p.type!=1 && p.author_id=?",[$id])){
            if($this->rowCount() > 0 ){
                $row=$this->fetchall();
                foreach ($row as $obj)
                {
    ...
    public function GetAll()
    {
        ...
        $ar=[];
        $ao=[];
        if($this->Query("SELECT p.id,p.heading,p.description,p.type,p.datetime,p.likes,p.comments,a.url FROM post p left join post_attachments a on p.id=a.post_id where p.approval_status=?",[1])){
            if($this->rowCount() > 0 ){
                $row=$this->fetchall();
                $i=0;
                foreach ($row as $obj)
        ...
        return ['status'=>'ok','qc'=>$qc,'qf'=>$qf,'qa'=>$qa,'qr'=>$qr,'qo'=>$qo,'ac'=>$ac,'af'=>$af,'aa'=>$aa,'ar'=>$ar,'ao'=>$ao];
